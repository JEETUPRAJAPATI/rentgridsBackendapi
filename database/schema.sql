...
-- Property Location Table (separate for normalized structure)
...
CREATE INDEX idx_property_features_feature_key ON property_features(feature_key);

CREATE INDEX idx_property_location_property_id ON property_location(property_id);
CREATE INDEX idx_property_location_city ON property_location(city);
CREATE INDEX idx_property_location_locality ON property_location(locality);
...
CREATE TRIGGER update_property_location_updated_at BEFORE UPDATE ON property_location FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE OR REPLACE FUNCTION sync_feature_legacy_fields()
RETURNS TRIGGER AS $$
BEGIN
    NEW.name := NEW.feature_key;
    RETURN NEW;
END;
$$ language 'plpgsql';

CREATE TRIGGER sync_feature_legacy_fields_trigger BEFORE INSERT OR UPDATE ON property_features FOR EACH ROW EXECUTE FUNCTION sync_feature_legacy_fields();